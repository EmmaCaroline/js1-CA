...
function generateJacketHtml(jacketFemale) {
  ...
  if (jacketFemale.onSale !== false) {
    ...
    jacketDiscountedPrice.textContent =
      "ON SALE: $ " + jacketFemale.discountedPrice;
  ...
  jacketFemaleContainer.append(
    heading,
    jacketImage,
    jacketPriceContainer,
    seeDetailsButton
  );
...
async function main() {
  ...
  const jacketsFemale = responseData.data.filter(
    (jacketFemale) => jacketFemale.gender === "Female"
  );
